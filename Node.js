...
const files = {}; // store files temporarily
      ...
      function acceptFile() {
        fetch('/download/' + fileId)
          ...
          .then(blob => {
            ...
            a.click();
            // Notify server that recipient accepted
...
io.on('connection', (socket) => {
  ...
  // Sender joins a room with the file ID
  socket.on('joinRoom', (fileId) => {
    socket.join(fileId);
  });

  // Recipient accepts file
  socket.on('accepted', (fileId) => {
    // Notify all sockets in that file room (the sender)
